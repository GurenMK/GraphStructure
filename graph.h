...
//     Precondition: (vertex_number(source) < size( )) and (vertex_number(target) < size( )).
...
//
//   void remove_edge(const Item&  source, const Item& target)
//     Precondition: (vertex_number(source) < size( )) and (vertex_number(target) < size( )).
...
//
//   set<Item> neighbors(const Item& v)) const
//     Precondition: (vertex_number[v] < size( )).
//     Postcondition: The return value is a set that contains all the vertices that
...
//
//    size_t vertex_number (const Item& v) const;
//      Precondition: v is in the labels array
//      Postcondition: vertex number associated with v has been returned
//
...
namespace main_savitch_15
{
    ...
    public:
        ...
        bool is_edge(const Item& source, const Item& target) const;
        std::set<Item> neighbors (const Item& v) const;
        Item operator[ ] (std::size_t vertex) const;
        std::size_t vertex_number (const Item& v) const;
    private:
        std::vector<std::set<Item> > edges;
